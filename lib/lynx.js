...
// #### @options.error_on_nodata    {Boolean} Throw an error if send is called with no metrics, defaults to TRUE ,  
//
...
function Lynx(host, port, options) {
  ...
  //
  // By default, throw an error if send was called with no data
  //
  this.error_on_nodata = false;
...
Lynx.prototype.send = function send(stats, sample_rate) {
  ...
  // And if lynx was configured to throw an error if there is nothing to send
  if(all_stats.length === 0 && this.error_on_nodata) {
    //
